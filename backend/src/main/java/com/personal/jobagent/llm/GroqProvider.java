...
public class GroqProvider implements LlmProvider {
    private final HttpClient client=HttpClient.newHttpClient(); private final ObjectMapper json;
    ...
    public Set<String> supportedModels(){return Set.of("llama-3.3-70b-versatile");}
    ...
    public LlmCompletion complete(LlmCompletionRequest request,Duration timeout) throws LlmProviderException {
        ...
        long start=System.currentTimeMillis(); try {
            ...
            HttpRequest call=HttpRequest.newBuilder().uri(URI.create(baseUrl+"/v1/chat/completions")).header("Authorization","Bearer "+apiKey).header("Content-Type","application/json").timeout(timeout).POST(HttpRequest.BodyPublishers.ofString(json.writeValueAsString(body))).build();
            ...
            JsonNode node=json.readTree(response.body()); String text=node.at("/choices/0/message/content").asText("");
