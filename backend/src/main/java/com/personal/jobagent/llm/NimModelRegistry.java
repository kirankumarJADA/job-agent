...
public class NimModelRegistry {
    ...
    /**
     * Rows backing GET /api/v1/models/nim/registry.
     *
     * <p>Columns are mapped explicitly instead of through {@code queryForList}, which hands
     * the raw JDBC driver objects to Jackson: a Postgres {@code text[]} arrives as a
     * {@code java.sql.Array} (pgjdbc {@code PgArray}) and jsonb as a {@code PGobject}.
     * Jackson bean-serialises those classes and calls their getters, including
     * {@code PgArray#getResultSet()} — which builds a live driver ResultSet — so rendering
     * the response failed with a JsonMappingException (HTTP 500). Reading through the shared
     * {@link JdbcConversions} helpers keeps the payload plain (list of strings / object),
     * like every other repository here, and matches why /models casts {@code notes::text}.
     */
    public List<Map<String,Object>> registryRows(){
        return db.query(
            "select provider_id,model_key,display_name,context_window,capabilities,enabled,notes from llm_models where provider_id='nim' order by model_key",
            (rs,n)->{
                Map<String,Object> row=new LinkedHashMap<>();
                row.put("provider_id",rs.getString("provider_id"));
                row.put("model_key",rs.getString("model_key"));
                row.put("display_name",rs.getString("display_name"));
                row.put("context_window",rs.getObject("context_window"));
                row.put("capabilities",JdbcConversions.readStringArray(rs,"capabilities"));
                row.put("enabled",rs.getBoolean("enabled"));
                row.put("notes",JdbcConversions.readJsonMap(rs,"notes",mapper));
                return row;
            });
    }
